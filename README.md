...
- **增量对比**：将本次页面内容与上次快照对比，提取新增、删除或调整内容；
...
| 路径 | 内容 |
| --- | --- |
| `data/snapshots/*_latest.json` | 最新正文、基准画像和变化记录 |
| `data/screenshots/*_latest.png` | 最近一次页面截图 |
| `data/logs/last_run.log` | 工作台最近一次执行日志 |
| `reports/daily_report_*.md` | 带时间戳的综合日报 |
...
- 当前没有自动重试机制，失败站点需要在下一次运行时重新处理；
- 不建议同时启动多个 GUI、CLI 或定时任务实例，以免覆盖同一快照或截图。
